...
\onslide<2>{\item<alert@2> We will focus on analysis technique suitable for
...
\only<2>{\small 
\begin{itemize}
\item Actually signals are acquired for a limited period
$T\rightarrow$ equivalent to the convolution with a box
function $G(t)$ with support $[0,T]$
\item In the
...
\item We need functions localized in time and scales $\Rightarrow$
...
%   \item The method distinguishes two orthogonal sets :
% \begin{equation*}
...
% \begin{center}
% \includegraphics[height=7.1cm]{strutturaMediaConVpNuova}
% \end{center}
% \end{frame}
