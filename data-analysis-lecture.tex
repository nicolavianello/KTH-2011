...
\onslide<2>{\item We will focus on analysis technique suitable for
...
\item In the
  Fourier space this is equivalent to the multiplication with 
the Fourier representation of a box function $\rightarrow \mathrm{sinc}(x) =
  \sin(x)/x \Rightarrow$\alert{leaking} some power from
...
\only<3>{
\begin{itemize}
\item Solution to the leakage: multiplication with an
...
\item It describes the frequency distribution of the \emph{power of
    the signal.} 
%i.e.  $E[|x(t)|^2]=\int_{-\infty}^{+\infty}S(f)dt$
...
\item Indeed if $k = k(f)$ then $g(t,\overline{r}) =
...
\only<3>{\item\textcolor{ta3chameleon}{\texttt{Wave vector}} is
...
\item \textcolor{tachameleon}{The probe distance $d$ must be less than
  aa wave length, less than a correlation length, but far enough the
...
\only<3>{\item Thus Fourier decomposition is not suitable for processes highly
  localized in time/space. We can use
  \textcolor{tachameleon}{\texttt{Wavelet
      Transform}}{\footnotesize \parencite{Farge:1992un}}. Functions
  well defined in time and scale

\begin{center}
\includegraphics[height=4cm]{morlet}
\end{center}
...
\centering{\includegraphics[height=4.5cm]{pdf_fluct}}
